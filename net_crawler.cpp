...
void HTMLParse(string & htmlResponse, vector<string> & imgurls, const string & host){
	...
	while (pos){
		...
		if (nextQ){
			...
			sscanf(pos, "%[^\"]", url);
			string surl = url;  //  转换成 string 类型，可以自动释放内存   
...
void main()
{
	...
	string urlStart = "http://luntanweb.net/read-htm-tid-5001609.html";  
	...
	//  使用广度遍历   
	//  提取网页中的超链接放入 hrefUrl 中，提取图片链接，下载图片。   
	BFS(urlStart);
	...
	while (hrefUrl.size() != 0){
		...
		cout << url << endl;
		BFS(url);                   //  遍历提取出来的那个网页，找它里面的超链接网页放入 hrefUrl ，下载它里面的文本，图片   
